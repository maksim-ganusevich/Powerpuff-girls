...
class ActionCatapult(Action):

    def __init__(self, considerations: List[Consideration]):
        super(ActionCatapult, self).__init__(considerations)

    def score(self, context: Context) -> float:
        context.desired_hex = Map().get_closest_in_list(context.get_curr_tank().position, Map().catapult)
        return super(ActionCatapult, self).score(context)

    def execute(self, context: Context) -> None:
        curr_tank = context.get_curr_tank()
        catapult = Map().get_closest_in_list(curr_tank.position, Map().catapult)
        if curr_tank.position == catapult:
            return

        prev_pos = curr_tank.position
        best_hex = context.path_reasoner.get_best_hex(context, catapult)
