...
class ActionLightRepair(Action):

    def __init__(self, considerations: List[Consideration]):
        super(ActionLightRepair, self).__init__(considerations)

    def score(self, context: Context) -> float:
        context.desired_hex = Map().get_closest_in_list(context.get_curr_tank().position, Map().light_repair)
        return super(ActionLightRepair, self).score(context)

    def execute(self, context: Context) -> None:
        curr_tank = context.get_curr_tank()
        repair_hex = Map().get_closest_in_list(curr_tank.position, Map().light_repair)
